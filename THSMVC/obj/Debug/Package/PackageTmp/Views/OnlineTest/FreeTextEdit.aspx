        ...
        function UpdateftQuestion() {
            ...
            if ($('#question').length && $('#question').val().length) {
                ...
                $.ajax({
                    type: "POST",
                    traditional: true,
                    url: "/OnlineTest/UpdateftQuestion",
                    data: {
...
<body>
    ...
    <div class="clear">
                    ...
                    <% if (Model.question.ToList().Count > cnt)
                       {
                           ...
                           {%>
                    <p>
                        <%=Html.TextBoxFor(m => m.question.ToList()[cnt].Answer, new { name = "ans" + (cnt + 1), id = "ftans" + (cnt + 1), size = "30" })%>Mandatory</p>
                        <%= Html.HiddenFor(m => m.question.ToList()[cnt].AnswerId, new { id = "hdnAId" +(cnt + 1) })%>
                    <%}
                           else
                           {%>
                     <p>
                        <%=Html.TextBoxFor(m => m.question.ToList()[cnt].Answer, new { name = "ans" + (cnt + 1), id = "ftans" + (cnt + 1), size = "30" })%>Optional</p>
                        <%= Html.HiddenFor(m => m.question.ToList()[cnt].AnswerId, new { id = "hdnAId" +(cnt + 1) })%>
                    <%}
                       }
                       else
                       { %>
                    <p>
                        <input type="text" name="ans<%=cnt+1 %>" size="30" id="ftans<%=cnt+1 %>" value="" />
                        Optional</p>
        ...
        <input type="button" class="rg_button_red" title="Save" value="Save Question" onclick="UpdateftQuestion();" />
    </div>
    <script type="text/javascript">
